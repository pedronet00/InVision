<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                <x-slot name="title">
                    {{ __('Adicionar um membro ao Time') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Adicione um novo membro à sua equipe, permitindo que ele colabore com você.') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600 dark:text-gray-400">
                            {{ __('Forneça o endereço de e-mail da pessoa que você gostaria de adicionar a esta equipe.') }}
                        </div>
                    </div>
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                ...
                <x-slot name="description">
                    {{ __('Essas pessoas foram convidadas para sua equipe e receberam um e-mail de convite. Eles podem ingressar na equipe aceitando o convite por e-mail.') }}
                </x-slot>
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                ...
                <x-slot name="description">
                    {{ __('Todos os participantes do time.') }}
                </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingLeavingTeam">
        ...
        <x-slot name="content">
            {{ __('Tem certeza que quer sair do Time?') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingTeamMemberRemoval">
        <x-slot name="title">
            {{ __('Remover membro do Time') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Tem certeza que quer remover este membro do Time?') }}
        </x-slot>
